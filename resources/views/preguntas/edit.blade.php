...
<div class="max-w-2xl mx-auto px-4 py-12">
    ...
    <form action="{{ route('preguntas.update', $pregunta) }}" method="POST" class="bg-white shadow rounded-lg p-6">
        ...
        <div class="mb-4">
            ...
            <input type="text" name="titulo" id="titulo" value="{{ old('titulo', $pregunta->titulo) }}" 
        ...
        <div class="mb-4">
            ...
            <textarea name="contenido" id="contenido" rows="6"
                      class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-green-500 focus:border-green-500"
                      required minlength="10" maxlength="2000">{{ old('contenido', $pregunta->contenido) }}</textarea>
    ...
    @if ($pregunta->estaRespondida())
        <div class="mt-8 p-4 bg-yellow-50 border border-yellow-200 rounded">
            <p class="text-yellow-800 text-sm">
                Esta pregunta ya ha sido respondida. Si necesitas hacer cambios importantes, contacta al administrador.
            </p>
        </div>
    @endif
</div>
