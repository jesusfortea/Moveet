    ...
    <div style="display: flex; flex-direction: column; align-items: center; width: 100%; padding: 20px 0;">
        <div style="width: 90%; max-width: 800px;">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 30px;">
                <h1 style="font-size: 2rem; font-weight: bold; margin: 0;">CREAR EVENTO</h1>
            ...
            <form action="{{ route('admin.eventos.guardar') }}" method="POST" style="background: #d0dbd9; padding: 25px; border-radius: 8px;">
                @csrf
                ...
                <div style="margin-bottom: 15px;">
                    ...
                    <input type="text" name="nombre" value="{{ old('nombre') }}" required minlength="3" maxlength="255" placeholder="Ej: Carrera Deportiva del Parque" style="width: 100%; padding: 10px 12px; border: 2px solid {{ $errors->has('nombre') ? '#c00' : '#999' }}; border-radius: 4px; font-size: 13px; box-sizing: border-box; background: {{ $errors->has('nombre') ? '#fff5f5' : '#fff' }};">
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-bottom: 15px;">
                    <div>
                        ...
                        <input type="number" name="ejeX" value="{{ old('ejeX') }}" required step="any" min="-180" max="180" placeholder="2.180559074338345" style="width: 100%; padding: 10px 12px; border: 2px solid {{ $errors->has('ejeX') ? '#c00' : '#999' }}; border-radius: 4px; font-size: 13px; box-sizing: border-box; background: {{ $errors->has('ejeX') ? '#fff5f5' : '#fff' }};">
                        @if ($errors->has('ejeX'))
                    ...
                    <div>
                        ...
                        <input type="number" name="ejeY" value="{{ old('ejeY') }}" required step="any" min="-90" max="90" placeholder="41.391197225342204" style="width: 100%; padding: 10px 12px; border: 2px solid {{ $errors->has('ejeY') ? '#c00' : '#999' }}; border-radius: 4px; font-size: 13px; box-sizing: border-box; background: {{ $errors->has('ejeY') ? '#fff5f5' : '#fff' }};">
                        @if ($errors->has('ejeY'))
                ...
                <div style="margin-bottom: 15px;">
                    ...
                    <input type="date" name="fecha_inicio" value="{{ old('fecha_inicio') }}" required style="width: 100%; padding: 10px 12px; border: 2px solid {{ $errors->has('fecha_inicio') ? '#c00' : '#999' }}; border-radius: 4px; font-size: 13px; box-sizing: border-box; background: {{ $errors->has('fecha_inicio') ? '#fff5f5' : '#fff' }};">
                ...
                <div style="margin-bottom: 20px;">
                    ...
                    <input type="date" name="fecha_fin" value="{{ old('fecha_fin') }}" required style="width: 100%; padding: 10px 12px; border: 2px solid {{ $errors->has('fecha_fin') ? '#c00' : '#999' }}; border-radius: 4px; font-size: 13px; box-sizing: border-box; background: {{ $errors->has('fecha_fin') ? '#fff5f5' : '#fff' }};">
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px;">
                    ...
                    <button type="submit" style="background: #8FA8A6; color: white; padding: 12px; border: none; border-radius: 4px; cursor: pointer; font-weight: 600; font-size: 13px;" onmouseover="this.style.background='#7a9a98'" onmouseout="this.style.background='#8FA8A6'">CREAR EVENTO</button>
                </div>
            </form>
        </div>
    </div>
