...
@section('title', 'Crear Misión - Admin')
    ...
    <div style="display: flex; flex-direction: column; align-items: center; width: 100%; padding: 20px 0;">
        <div style="width: 90%; max-width: 800px;">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 30px;">
                <h1 style="font-size: 2rem; font-weight: bold; margin: 0;">CREAR MISIÓN</h1>
            ...
            <form action="{{ route('admin.misiones.guardar') }}" method="POST" style="background: #d0dbd9; padding: 25px; border-radius: 8px;">
                @csrf
                ...
                <div style="margin-bottom: 15px;">
                    ...
                    <select name="evento_id" required style="width: 100%; padding: 10px 12px; border: 2px solid {{ $errors->has('evento_id') ? '#c00' : '#999' }}; border-radius: 4px; font-size: 13px; box-sizing: border-box; background: {{ $errors->has('evento_id') ? '#fff5f5' : '#fff' }};">
                        ...
                        @foreach ($eventos as $evento)
                            <option value="{{ $evento->id }}" {{ old('evento_id') == $evento->id ? 'selected' : '' }}>{{ $evento->nombre }}</option>
                        @endforeach
                    </select>
                ...
                <div style="margin-bottom: 15px;">
                    ...
                    <input type="text" name="nombre" value="{{ old('nombre') }}" required minlength="3" maxlength="255" placeholder="Mínimo 3 caracteres" style="width: 100%; padding: 10px 12px; border: 2px solid {{ $errors->has('nombre') ? '#c00' : '#999' }}; border-radius: 4px; font-size: 13px; box-sizing: border-box; background: {{ $errors->has('nombre') ? '#fff5f5' : '#fff' }};">
                ...
                <div style="margin-bottom: 15px;">
                    ...
                    <textarea name="descripcion" required minlength="10" maxlength="1000" placeholder="Mínimo 10 caracteres" style="width: 100%; padding: 10px 12px; border: 2px solid {{ $errors->has('descripcion') ? '#c00' : '#999' }}; border-radius: 4px; font-size: 13px; box-sizing: border-box; background: {{ $errors->has('descripcion') ? '#fff5f5' : '#fff' }}; font-family: Arial, sans-serif; min-height: 80px; resize: vertical;"></textarea>
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-bottom: 15px;">
                    <div>
                        ...
                        <input type="number" name="ejeX" value="{{ old('ejeX') }}" required step="0.0001" min="-180" max="180" placeholder="-3.7038" style="width: 100%; padding: 10px 12px; border: 2px solid {{ $errors->has('ejeX') ? '#c00' : '#999' }}; border-radius: 4px; font-size: 13px; box-sizing: border-box; background: {{ $errors->has('ejeX') ? '#fff5f5' : '#fff' }};">
                        @if ($errors->has('ejeX'))
                    ...
                    <div>
                        ...
                        <input type="number" name="ejeY" value="{{ old('ejeY') }}" required step="0.0001" min="-90" max="90" placeholder="40.4168" style="width: 100%; padding: 10px 12px; border: 2px solid {{ $errors->has('ejeY') ? '#c00' : '#999' }}; border-radius: 4px; font-size: 13px; box-sizing: border-box; background: {{ $errors->has('ejeY') ? '#fff5f5' : '#fff' }};">
                        @if ($errors->has('ejeY'))
                ...
                <div style="margin-bottom: 15px;">
                    <label style="display: block; font-weight: 600; margin-bottom: 5px; color: #333; font-size: 13px;">Puntos <span style="color: #c00;">*</span></label>
                    <input type="number" name="puntos" value="{{ old('puntos') }}" required min="1" max="9999" placeholder="100" style="width: 100%; padding: 10px 12px; border: 2px solid {{ $errors->has('puntos') ? '#c00' : '#999' }}; border-radius: 4px; font-size: 13px; box-sizing: border-box; background: {{ $errors->has('puntos') ? '#fff5f5' : '#fff' }};">
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-bottom: 20px;">
                    <div style="display: flex; align-items: center;">
                        <input type="checkbox" name="premium" id="premium" value="1" {{ old('premium') ? 'checked' : '' }} style="margin-right: 8px;">
                        <label for="premium" style="color: #333; font-size: 13px; cursor: pointer;">Misión Premium</label>
                    </div>
                    <div style="display: flex; align-items: center;">
                        <input type="checkbox" name="semanal" id="semanal" value="1" {{ old('semanal') ? 'checked' : '' }} style="margin-right: 8px;">
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px;">
                    ...
                    <button type="submit" style="background: #8FA8A6; color: white; padding: 12px; border: none; border-radius: 4px; cursor: pointer; font-weight: 600; font-size: 13px;" onmouseover="this.style.background='#7a9a98'" onmouseout="this.style.background='#8FA8A6'">CREAR MISIÓN</button>
                </div>
            </form>
        </div>
    </div>
