...
@section('title', 'Eliminar Recompensa')
    ...
    <div style="display: flex; flex-direction: column; align-items: center; width: 100%; padding: 20px 0;">
        <div style="width: 90%; max-width: 700px;">
            ...
            <div style="background: #f8f1f1; border: 2px solid #cc0000; padding: 25px; border-radius: 8px;">
                <p style="margin-top: 0; font-size: 15px; color: #600;">Vas a eliminar esta recompensa:</p>
                <div style="background: white; padding: 16px; border-radius: 6px; margin-bottom: 18px;">
                    <div style="font-weight: 700; font-size: 18px; margin-bottom: 8px;">{{ $recompensa->nombre }}</div>
                    <div style="color: #666; margin-bottom: 6px;">{{ $recompensa->descripcion }}</div>
                    <div style="color: #333; font-size: 13px;">Tipo: {{ $recompensa->tipo }} | Puntos: {{ $recompensa->puntos_necesarios }} | Premium: {{ $recompensa->premium ? 'Sí' : 'No' }}</div>
                </div>

                <form action="{{ route('admin.recompensas.confirmar-eliminar', $recompensa) }}" method="POST">
                    @csrf
                    @method('DELETE')
