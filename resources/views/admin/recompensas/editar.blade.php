...
@section('title', 'Editar Recompensa - Admin')
        ...
        <div style="width: 90%; max-width: 800px;">
            ...
            <form action="{{ route('admin.recompensas.actualizar', $recompensa) }}" method="POST" style="background: #d0dbd9; padding: 25px; border-radius: 8px;">
                ...
                <div style="margin-bottom: 15px;">
                    ...
                    <select name="pase_de_paseo_id" required style="width: 100%; padding: 10px 12px; border: 2px solid {{ $errors->has('pase_de_paseo_id') ? '#c00' : '#999' }}; border-radius: 4px; font-size: 13px; box-sizing: border-box; background: {{ $errors->has('pase_de_paseo_id') ? '#fff5f5' : '#fff' }};">
                        ...
                        @foreach ($pases as $pase)
                            <option value="{{ $pase->id }}" @if(old('pase_de_paseo_id', $recompensa->pase_de_paseo_id) == $pase->id) selected @endif>{{ $pase->nombre }}</option>
                        @endforeach
                    </select>
                ...
                <div style="margin-bottom: 15px;">
                    ...
                    <input type="text" name="nombre" value="{{ old('nombre', $recompensa->nombre) }}" required minlength="3" maxlength="255" placeholder="Ej: Moneda de Oro" style="width: 100%; padding: 10px 12px; border: 2px solid {{ $errors->has('nombre') ? '#c00' : '#999' }}; border-radius: 4px; font-size: 13px; box-sizing: border-box; background: {{ $errors->has('nombre') ? '#fff5f5' : '#fff' }};">
                ...
                <div style="margin-bottom: 15px;">
                    ...
                    <textarea name="descripcion" required minlength="10" maxlength="1000" placeholder="Describe la recompensa" style="width: 100%; padding: 10px 12px; border: 2px solid {{ $errors->has('descripcion') ? '#c00' : '#999' }}; border-radius: 4px; font-size: 13px; box-sizing: border-box; background: {{ $errors->has('descripcion') ? '#fff5f5' : '#fff' }}; font-family: Arial, sans-serif; min-height: 80px; resize: vertical;">{{ old('descripcion', $recompensa->descripcion) }}</textarea>
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-bottom: 15px;">
                    ...
                    <div>
                        ...
                        <input type="number" name="nivel_necesario" value="{{ old('nivel_necesario', $recompensa->nivel_necesario) }}" required min="1" max="100" placeholder="1" style="width: 100%; padding: 10px 12px; border: 2px solid {{ $errors->has('nivel_necesario') ? '#c00' : '#999' }}; border-radius: 4px; font-size: 13px; box-sizing: border-box; background: {{ $errors->has('nivel_necesario') ? '#fff5f5' : '#fff' }};">
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-bottom: 15px;">
                    <div>
                        ...
                        <select name="tipo" required style="width: 100%; padding: 10px 12px; border: 2px solid {{ $errors->has('tipo') ? '#c00' : '#999' }}; border-radius: 4px; font-size: 13px; box-sizing: border-box; background: {{ $errors->has('tipo') ? '#fff5f5' : '#fff' }};">
                            ...
                            <option value="normal" @if(old('tipo', $recompensa->tipo) === 'normal') selected @endif>Normal</option>
                            <option value="especial" @if(old('tipo', $recompensa->tipo) === 'especial') selected @endif>Especial</option>
                            <option value="legendaria" @if(old('tipo', $recompensa->tipo) === 'legendaria') selected @endif>Legendaria</option>
                        </select>
                        @if ($errors->has('tipo'))
                    ...
                    <div>
                        ...
                        <div style="display: flex; align-items: center; height: 36px;">
                            <input type="checkbox" name="premium" value="1" @if(old('premium', $recompensa->premium)) checked @endif style="width: 18px; height: 18px; cursor: pointer;">
