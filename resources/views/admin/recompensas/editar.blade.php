    ...
    <div style="display: flex; flex-direction: column; align-items: center; width: 100%; padding: 20px 0;">
        <div style="width: 90%; max-width: 850px;">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 30px; gap: 16px; flex-wrap: wrap;">
                <h1 style="font-size: 2rem; font-weight: bold; margin: 0;">EDITAR RECOMPENSA</h1>
                <div style="display: flex; gap: 10px; flex-wrap: wrap;">
                    ...
                    <a href="{{ route('admin.recompensas.eliminar', $recompensa) }}" style="background: white; color: #8FA8A6; padding: 12px 24px; border-radius: 6px; text-decoration: none; font-weight: 600; border: 1px solid #8FA8A6;">ELIMINAR RECOMPENSA</a>
                </div>
            </div>
            ...
            <form action="{{ route('admin.recompensas.actualizar', $recompensa) }}" method="POST" enctype="multipart/form-data" style="background: #d0dbd9; padding: 25px; border-radius: 8px;">
                ...
                <div style="margin-bottom: 15px;">
                    ...
                    <input type="text" name="nombre" value="{{ old('nombre', $recompensa->nombre) }}" required maxlength="255" style="width: 100%; padding: 10px 12px; border: 2px solid {{ $errors->has('nombre') ? '#c00' : '#999' }}; border-radius: 4px; font-size: 13px; box-sizing: border-box; background: {{ $errors->has('nombre') ? '#fff5f5' : '#fff' }};">
                </div>
                ...
                <div style="margin-bottom: 15px;">
                    ...
                    <textarea name="descripcion" required maxlength="255" rows="4" style="width: 100%; padding: 10px 12px; border: 2px solid {{ $errors->has('descripcion') ? '#c00' : '#999' }}; border-radius: 4px; font-size: 13px; box-sizing: border-box; background: {{ $errors->has('descripcion') ? '#fff5f5' : '#fff' }};">{{ old('descripcion', $recompensa->descripcion) }}</textarea>
                </div>
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-bottom: 15px;">
                    <div>
                        ...
                        <select id="tipo_recompensa" name="tipo" required style="width: 100%; padding: 10px 12px; border: 2px solid {{ $errors->has('tipo') ? '#c00' : '#999' }}; border-radius: 4px; font-size: 13px; box-sizing: border-box; background: {{ $errors->has('tipo') ? '#fff5f5' : '#fff' }};">
                            <option value="tienda" {{ old('tipo', $recompensa->tipo) === 'tienda' ? 'selected' : '' }}>Tienda</option>
                            <option value="pase_de_paseo" {{ old('tipo', $recompensa->tipo) === 'pase_de_paseo' ? 'selected' : '' }}>Pase de paseo</option>
                        </select>
                    </div>
                    <div>
                ...
                <div id="pase_selector_wrap" style="margin-bottom: 15px; display: none;">
                    ...
                    <select id="pase_de_paseo_id" name="pase_de_paseo_id" style="width: 100%; padding: 10px 12px; border: 2px solid {{ $errors->has('pase_de_paseo_id') ? '#c00' : '#999' }}; border-radius: 4px; font-size: 13px; box-sizing: border-box; background: {{ $errors->has('pase_de_paseo_id') ? '#fff5f5' : '#fff' }};">
                        ...
                        @foreach($pases as $pase)
                            <option value="{{ $pase->id }}" {{ (string) old('pase_de_paseo_id', $recompensa->pase_de_paseo_id) === (string) $pase->id ? 'selected' : '' }}>
                                {{ $pase->nombre }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-bottom: 15px;">
                    ...
                    <div>
                        ...
                        <input type="number" name="nivel_necesario" value="{{ old('nivel_necesario', $recompensa->nivel_necesario) }}" required min="1" style="width: 100%; padding: 10px 12px; border: 2px solid {{ $errors->has('nivel_necesario') ? '#c00' : '#999' }}; border-radius: 4px; font-size: 13px; box-sizing: border-box; background: {{ $errors->has('nivel_necesario') ? '#fff5f5' : '#fff' }};">
                    </div>
                </div>
                ...
                <div style="margin-bottom: 18px; display: flex; flex-direction: column; gap: 10px; background: #eef4f3; padding: 12px 14px; border-radius: 6px;">
                    <label style="display: flex; align-items: center; gap: 10px; font-weight: 600; color: #333; font-size: 13px;">
                        <input type="checkbox" name="premium" value="1" {{ old('premium', $recompensa->premium) ? 'checked' : '' }} style="width: 18px; height: 18px;">
                        Producto premium
                    </label>
                    <label style="display: flex; align-items: center; gap: 10px; font-weight: 600; color: #333; font-size: 13px;">
                        <input type="checkbox" name="visible_en_tienda" value="1" {{ old('visible_en_tienda', $recompensa->visible_en_tienda) ? 'checked' : '' }} style="width: 18px; height: 18px;">
