...
@section('title', 'Eliminar Lugar - Admin')
    ...
    <div style="display: flex; flex-direction: column; align-items: center; width: 100%; padding: 20px 0;">
        <div style="width: 90%; max-width: 800px;">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 30px;">
                <h1 style="font-size: 2rem; font-weight: bold; margin: 0;">ELIMINAR LUGAR</h1>
            ...
            <div style="background: #d0dbd9; padding: 25px; border-radius: 8px;">
                <div style="background: #fff3cd; border: 2px solid #ffc107; color: #856404; padding: 15px; border-radius: 6px; margin-bottom: 20px;">
                    ...
                    <p style="margin: 0; font-size: 13px;">Esta acción es irreversible. Se eliminará el lugar permanentemente.</p>
                </div>

                <form style="background: white; padding: 20px; border-radius: 6px; margin-bottom: 20px;">
                    <div style="margin-bottom: 15px;">
                        <label style="display: block; font-weight: 600; margin-bottom: 5px; color: #333; font-size: 13px;">Nombre del lugar</label>
                        <input type="text" value="{{ $lugar->nombre }}" disabled style="width: 100%; padding: 10px 12px; border: 2px solid #ccc; border-radius: 4px; font-size: 13px; box-sizing: border-box; background: #f5f5f5; color: #666;">
                    </div>

                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-bottom: 20px;">
                        <div>
                            <label style="display: block; font-weight: 600; margin-bottom: 5px; color: #333; font-size: 13px;">Coordenada X</label>
                            <input type="number" value="{{ $lugar->ejeX }}" disabled style="width: 100%; padding: 10px 12px; border: 2px solid #ccc; border-radius: 4px; font-size: 13px; box-sizing: border-box; background: #f5f5f5; color: #666;">
                        </div>
                        <div>
                            <label style="display: block; font-weight: 600; margin-bottom: 5px; color: #333; font-size: 13px;">Coordenada Y</label>
                            <input type="number" value="{{ $lugar->ejeY }}" disabled style="width: 100%; padding: 10px 12px; border: 2px solid #ccc; border-radius: 4px; font-size: 13px; box-sizing: border-box; background: #f5f5f5; color: #666;">
                        </div>
                    </div>
                </form>
                ...
                <div style="background: #f9f9f9; border-left: 3px solid #c00; padding: 15px; border-radius: 4px; margin-bottom: 20px;">
                    <p style="margin: 0; font-size: 13px; color: #666;">
                        ¿Estás seguro de que deseas eliminar este lugar? Una vez eliminado, no se puede recuperar.
                    </p>
                </div>

                <form action="{{ route('admin.lugares.confirmar-eliminar', $lugar) }}" method="POST" style="display: flex; gap: 12px;">
                    @csrf
                    @method('DELETE')
